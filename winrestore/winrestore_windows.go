...
// Package winrestore provides facilities for searching through Windows System Restore Points for backed-up Tailscale state information.
package winrestore
...
import (
	...
	"tailscale.com/paths"
	"tailscale.com/winrestore/vss"
)

var StopWalking error = errors.New("Stop walking")

type SnapshotProperties = vss.SnapshotProperties
...
type WalkSnapshotsFunc func(path string, props SnapshotProperties) error
...
func mountSnapshotDevice(snap SnapshotProperties, mountPath string) (mountedSnapshot, error) {
